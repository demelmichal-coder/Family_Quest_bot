...
test("vytvori novou odmenu", async () => {
  ...
  await user.type(inputs[2], "Zoo");
  await user.type(inputs[3], "Vylet");
  await user.clear(screen.getByRole("spinbutton", { name: /cena ve zlate/i }));
  await user.type(screen.getByRole("spinbutton", { name: /cena ve zlate/i }), "12");
  await user.click(screen.getByRole("button", { name: "Vytvorit odmenu" }));
...
test("vygeneruje navrh ukolu pomoci AI", async () => {
  ...
  expect(screen.getByText("AI navrh")).toBeInTheDocument();
  expect(screen.getByText(/Vyprava za cistym pokojem/)).toBeInTheDocument();
