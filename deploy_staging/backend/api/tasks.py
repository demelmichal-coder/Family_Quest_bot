...
def create_task(
    ...
    if new_task.user_id:
        ...
        if assignee:
            msg = f"🎯 Dostals novou misi: <b>{new_task.title}</b> (+{new_task.xp} XP, +{new_task.gold} gold)"
            background_tasks.add_task(asyncio.run, send_telegram_message(assignee.telegram_id, msg))
...
def create_tasks_bulk(
    ...
    for uid, titles in titles_by_user.items():
        ...
        if assignee:
            ...
            msg = f"🎯 Mas {len(titles)} novych misi: <b>{preview}</b>"
            background_tasks.add_task(asyncio.run, send_telegram_message(assignee.telegram_id, msg))
