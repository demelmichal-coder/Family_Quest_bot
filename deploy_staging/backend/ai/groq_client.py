...
async def rewrite_task_to_game(task_text: str, style: str = DEFAULT_REWRITE_STYLE) -> Tuple[str, int]:
    ...
    if "|" in content:
        try:
            name, xp = content.split("|", 1)
            return name.strip(), int(xp.strip())
        except ValueError:
            pass # Záchrana: Pokud model nevrátí čisté číslo, přejde se na default
...
async def rewrite_reward_to_shop_item(
    ...
    if "|" in content:
        try:
            name, cost = content.split("|", 1)
            return name.strip(), int(cost.strip())
        except ValueError:
